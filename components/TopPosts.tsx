    ...
    <>
      <div className="card" style={{ padding: 20 }}>
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>Top 4 Posts</div>
          <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 2 }}>
            Ταξινόμηση βάσει προβολών · κλικ για λεπτομέρειες
          </div>
        </div>
            ...
            return (
              <div
                ...
                onMouseEnter={(e) => {
                  ...
                  el.style.boxShadow = `0 0 20px ${rank.glow}, 0 8px 24px rgba(0,0,0,0.3)`
                }}
                onMouseLeave={(e) => {
                  const el = e.currentTarget as HTMLElement
                  el.style.transform = 'none'
                  el.style.boxShadow = `0 0 12px ${rank.glow}`
                }}
              >
                {/* Rank badge */}
                <div
                ...
                >
                  <ThumbnailCell post={post} size={300} />
                </div>
                ...
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 10, color: 'var(--text-3)', marginBottom: 1 }}>{primaryLabel}</div>
...
function MiniStat({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: string; color: string }) {
  return (
    <div style={{ background: 'var(--bg-subtle)', borderRadius: 5, padding: '5px 7px', display: 'flex', flexDirection: 'column', gap: 2 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 3, color, opacity: 0.85 }}>
        {icon}
        <span style={{ fontSize: 9, color: 'var(--text-3)', textTransform: 'uppercase' }}>{label}</span>
      </div>
